...
public class VideoCompress {
    ...
    private static class VideoCompressTask extends AsyncTask<String, Float, Boolean> {
        ...
        protected Boolean doInBackground(String... paths) {
            Log.d("COMPRESSOR1 path 0", paths[0]);
            Log.d("COMPRESSOR1 path 1", paths[1]);
            return VideoController.getInstance().convertVideo(paths[0], paths[1], mQuality,
                    new VideoController.CompressProgressListener() {
                        ...
                        public void onProgress(float percent) {
                            Log.d("COMPRESSOR1", "doInBackground");
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (mListener != null) {
                if (result) {
                    Log.d("COMPRESSOR1", "onSuccess");
                    mListener.onSuccess();
                } else {
                    Log.d("COMPRESSOR1", "onfail");
                    mListener.onFail();
                }
            }
        }
